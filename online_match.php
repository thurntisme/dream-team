...
// Support both league matches (by uuid) and direct challenges (by opponent parameter)
$match_uuid = $_GET['uuid'] ?? ($_GET['match_uuid'] ?? null);
$opponent_uuid = $_GET['opponent'] ?? null;

if (!$match_uuid && !$opponent_uuid) {
    $renderError('Missing match identifier or opponent.');
    return;
}
...
try {
    ...
    // Handle direct challenge (opponent UUID provided)
    if ($opponent_uuid) {
        // Fetch opponent team data from user_club
        ...
        // Setup match-like structure for direct challenge
        $match = [
            'uuid' => null,
            'home_user_uuid' => $user_uuid,
            'away_user_uuid' => $opponent_uuid,
            'home_team_name' => $user_data['club_name'] ?? 'Your Club',
            'away_team_name' => $opponent_data['club_name'] ?? 'Opponent Club',
            'status' => 'pending',
        ...
        $is_user_match = true;
        $home_roster = null;
        $away_roster = json_decode($opponent_data['team'] ?? '[]', true) ?? [];
        $user_id_resolved = 0;
        
        // Parse teams (will be displayed using displayTeamLineup function)
        ...
        $stmtMatch->bindValue(':match_uuid', $match_uuid, SQLITE3_TEXT);
